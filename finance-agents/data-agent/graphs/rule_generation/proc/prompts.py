...
def build_understanding_prompt(context: dict[str, Any]) -> str:
    ...
    return (
        ...
        "当用户说某个字段“作为/用于/充当”某个业务用途、核对用途、识别用途、比对用途、统计用途或后续规则用途时，"
        "该字段通常是当前整理结果需要产出的列；必须同时建 source_references 和 output_specs，output_specs.name 使用用户说的字段名，kind=passthrough 或 rename，并用 source_ref_ids 指向对应源字段。\n"
        "只有纯过滤条件、纯排序条件、纯关联键且用户未要求在结果中展示的字段，才可以只进入 source_references/business_rules 而不进入 output_specs。\n"
...
def build_ir_repair_prompt(context: dict[str, Any], *, failures: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "- 如果用户说某个字段“作为/用于/充当”某个业务用途、核对用途、识别用途、比对用途、统计用途或后续规则用途，"
        "该字段通常是整理结果需要产出的列；请创建 output_specs，kind=passthrough 或 rename，name 使用用户说的字段名，source_ref_ids 指向对应 source_references。\n"
        "- 纯过滤条件、纯排序条件、纯关联键且用户未要求在结果展示的字段，不要因为它们参与规则就强行输出。\n"
        "- 不把输出字段别名当作源数据字段。\n"
