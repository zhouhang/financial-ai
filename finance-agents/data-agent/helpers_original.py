...
async def _analyze_single_file(uploaded_file: dict, complexity_info: dict) -> dict:
    ...
    # 构建包含样例数据的错误消息（使用 HTML 表格）
    msg_parts = ["⚠️ 请重新上传两个文件，文件数据样例如下：\n"]
...
def build_format_error_message(
    ...
    if "包含" in reason and "sheet" in reason:
        # 多 sheet 错误
        msg_parts.append(f"⚠️ 检测到文件 {filename} 有多个sheet，每个文件只能有一个sheet，请重新上传两个文件，文件数据样例如下：\n")
    else:
        # 格式错误（缺少表头、没有数据行、文件不存在等）
        msg_parts.append(f"⚠️ 检测到文件 {filename} 数据格式不符合规范，请重新上传两个文件，文件数据样例如下：\n")
