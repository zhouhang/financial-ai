...
3. 采集过程尽量不触发风控。
4. 一旦触发风控（验证码/滑块/安全校验/手机验证等，且可发生在登录或登录后任意操作步骤），不直接失败：Tally 经**该对账任务配置的通知通道（钉钉/飞书/企微）**把含一次性链接的消息发给**责任人**，责任人打开链接在云端页面上远程完成验证。
...
## 设计定位与 2026-05-24 真机验证修订

2026-05-24 在真千牛账号上跑通了一次完整验证(单枪旗舰店,needs_reauth profile):runner 自动用凭证登录 → 千牛弹出 `iv/mini/identity_verify.htm`(**发到商家手机的短信验证码**) → 阶段2 正确识别为 RISK_VERIFICATION、保持 Chrome 不关、等满 5 分钟、未完成则按 RISK_VERIFICATION 失败。阶段2 行为验证通过;但暴露了"操作员自己能完成验证"这一前提对短信码不成立。据此明确四点定位,并修订 handoff 设计:

- **定位1 通用性**:浏览器采集不只针对千牛这种强风控站,而是面向所有需要采集的网站。风控兜底要做成**通用能力**(风控标记/挑战类型按 playbook 可配),千牛只是最硬的实例。
- **定位2 风控无处不在、形式不限**:风控不只在登录,**登录后操作系统内功能也可能触发**;形式以手机验证码为主,但不限于(滑块、安全校验、app 确认等)。→ 风控检测必须覆盖**任意步骤**(在步骤循环外层统一拦截,而非只 login/navigate);远程接管要支持**任意形式挑战**,即给责任人真正的交互式控制(看画面 + 鼠键输入)。
- **定位3 例外路径**:正常是全自动(每日定时采 + 对账任务伴随采),风控中断影响"自动"体感但**偶发可接受**。→ handoff 是低频例外路径,不必追求极致实时,够用可靠即可。
- **定位4 触达责任人靠对账任务通道、责任人能解决(核心修正)**:handoff 触达对象 = **该对账任务配置的通知通道(钉钉/飞书/企微,复用已建适配器)对应的责任人**,而非泛指操作员;责任人有权限、能登录、能完成验证(短信码在其手机上 / 能拖滑块)。隐含**配置约束**:每个店铺对账任务的责任人必须是"对该店有权限/有手机"的人,否则短信码仍无解。

**对原 handoff 设计(阶段3/4)的三处修正**:① 触达对象明确为"对账任务通道的责任人",通道复用已建的钉钉/飞书/企微;② 风控检测覆盖任意步骤、任意形式;③ 做成通用能力(不止千牛)。原"云端 handoff session + 一次性链接 + 远程截图/输入接管"的大方向不变。

**远程输入技术分叉(真机待定)**:短信码这类只是填字符,Playwright 级输入转发即可;滑块类能否靠 Playwright 输入通过、还是要升级 OS 级远程桌面,仍需专门真机验证(原设计已标的升级路径)。
...
当采集页面触发风控(验证码/滑块/安全校验/手机验证等)时，新增“人工验证会话”。

触发条件(可发生在**登录或登录后任意操作步骤**，见定位2)：

- 页面出现滑块 / 拖动滑块 / 安全校验 / 手机验证 / 验证码等强风控标记。
- 登录后仍停留在登录域名或安全验证页。
- 登录后在系统内操作(导航/点击/下载等)中途弹出风控。
- playbook 无法继续，但浏览器页面仍然可被人工处理。

处理流程：

1. browser-agent 在**任意步骤**检测到风控标记(不限登录、不限形式)。
...
5. 云端 Tally 经**该对账任务配置的通知通道(钉钉/飞书/企微，复用已建适配器)**给**责任人**发送消息。
6. 消息包含一个短期有效、一次性使用的云端 Tally 链接。
7. 责任人打开链接后，看到当前浏览器页面画面。
8. 责任人可以点击、拖拽、输入，完成滑块、手机验证码或其他验证(短信码填字符即可，Playwright 级输入足够;滑块类的输入方式见远程接管的升级路径)。
...
5. 云端经对账任务配置的通知通道(钉钉/飞书/企微)发消息给责任人。
6. 责任人打开一次性链接并领取 session。
7. 云端页面展示当前浏览器截图流。
8. 责任人操作页面，输入事件经云端转发给 browser-agent。
...
- 经对账任务配置的通知通道(钉钉/飞书/企微)给责任人发送消息(复用已建通知适配器)。
