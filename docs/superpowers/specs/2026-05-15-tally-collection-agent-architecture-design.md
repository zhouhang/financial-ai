...
| `profile_status` | `none` / `active` / `needs_reauth` / `risk_blocked`——**仅描述登录态/风控态**：`needs_reauth`=登录失效（`AUTH_EXPIRED`），`risk_blocked`=风控拦截（`RISK_VERIFICATION`）|
...
P4 / P7 可放到首店跑通之后 sprint。首店阶段创作 playbook 走临时人工流程：Operator 本机用 AI 编码工具（如 Cline + DeepSeek-V4 Pro）跑出 playbook + sample，手动上传给 Playbook Registry。这是 Operator 个人开发工具流，**不属于生产架构组件**。P4 上线后切换到 Authoring Worker 自动化。
