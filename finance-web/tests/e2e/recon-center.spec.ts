...
async function selectDatasetInDropdown(page: Page, sectionTitle: '左侧原始数据' | '右侧原始数据', datasetName: string) {
  const card = page
...
async function createReconScheme(
  ...
  await page.getByLabel('对账目标').fill('核对左右数据金额以及业务主键一致性。');
  ...
  await page.getByLabel('左侧数据描述').fill('将左侧数据整理成订单维度明细并保留业务主键、金额、时间。');
  await page.getByLabel('右侧数据描述').fill('将右侧数据整理成订单维度明细并与左侧匹配。');
  await expect(page.getByRole('button', { name: '下一步' })).toBeEnabled();
  await page.getByRole('button', { name: '下一步' }).click();

  await page.getByRole('button', { name: 'AI生成整理配置' }).click();
  await expect(page.getByText('AI 正在生成整理配置，请稍候…')).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText('AI 正在生成整理配置，请稍候…')).toHaveCount(0, { timeout: 180_000 });
  await expect(page.getByLabel('数据整理配置')).toHaveValue(/\S/, { timeout: 15_000 });

  await page.getByRole('button', { name: '试跑验证' }).click();
  await expect(page.getByText('AI 正在试跑数据整理，请稍候…')).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText('AI 正在试跑数据整理，请稍候…')).toHaveCount(0, { timeout: 120_000 });
  ...
  await expect(page.getByText('AI 正在生成对账逻辑，请稍候…')).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText('AI 正在生成对账逻辑，请稍候…')).toHaveCount(0, { timeout: 120_000 });
  await expect(page.getByLabel('数据对账逻辑')).toHaveValue(/\S/, { timeout: 15_000 });

  await page.getByRole('button', { name: '试跑验证' }).click();
  await expect(page.getByText('AI 正在试跑数据对账，请稍候…')).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText('AI 正在试跑数据对账，请稍候…')).toHaveCount(0, { timeout: 120_000 });
  await expect(page.getByText('对账结果摘要')).toBeVisible({ timeout: 30_000 });
  await page.getByRole('button', { name: '下一步' }).click();

  await expect(page.getByText('方案摘要')).toBeVisible();
...
async function createRunPlanForScheme(
  ...
  await expect(page.getByLabel('对账方案')).toBeVisible();
  await page.getByLabel('对账方案').selectOption({ label: schemeName });
...
test.describe.serial('对账任务关键路径', () => {
  ...
  test('运行记录中展示异常看板入口', async ({ page }) => {
    ...
    const { planCode, planName } = await createRunPlanForScheme(
      page,
      authSession,
      sharedScheme.schemeName,
      sharedScheme.schemeCode,
...
test('对账中心可打开并进入新增对账方案第二步', async ({ page }) => {
  ...
  await page.getByLabel('对账目标').fill('核对左右数据集中的订单金额是否一致。');

  const datasetSelectors = page.getByRole('button', { name: '请选择数据集（可多选）' });

  await datasetSelectors.first().click();
  await page.getByRole('checkbox').first().check();
  await page.getByRole('button', { name: '确定' }).click();

  await page.getByRole('button', { name: '请选择数据集（可多选）' }).click();
  await page.getByRole('checkbox').first().check();
  await page.getByRole('button', { name: '确定' }).click();
...
test('对账方案可完成真实生成并保存运行计划后查看运行记录', async ({ page }) => {
  ...
  await page.getByLabel('方案名称').fill(schemeName);
  await page.getByLabel('对账目标').fill('将左右两侧数据整理成同一口径的订单金额明细，按业务主键匹配并校验金额是否一致。');
  await selectDatasetInDropdown(page, '左侧原始数据', selectedDataset.datasetName);
  await selectDatasetInDropdown(page, '右侧原始数据', selectedDataset.datasetName);
  await page.getByLabel('左侧数据描述').fill('将左侧数据整理成订单维度明细，保留唯一业务主键、业务日期、金额和来源名称。');
  await page.getByLabel('右侧数据描述').fill('将右侧数据整理成订单维度明细，保留唯一业务主键、业务日期、金额和来源名称。');
  ...
  await page.getByRole('button', { name: 'AI生成整理配置' }).click();
  await expect(page.getByText('AI 正在生成整理配置，请稍候…')).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText('AI 正在生成整理配置，请稍候…')).toHaveCount(0, { timeout: 120_000 });
  await expect(page.getByLabel('数据整理配置')).toHaveValue(/\S[\s\S]*/, { timeout: 15_000 });
  ...
  await expect(page.getByText('AI 正在生成对账逻辑，请稍候…')).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText('AI 正在生成对账逻辑，请稍候…')).toHaveCount(0, { timeout: 120_000 });
  await expect(page.getByLabel('数据对账逻辑')).toHaveValue(/\S[\s\S]*/, { timeout: 15_000 });
  ...
  await page.getByRole('button', { name: '新增运行计划' }).first().click();
  await expect(page.getByText('为方案补充调度、时间口径、协作通道与责任人')).toBeVisible();
  await page.getByLabel('对账方案').selectOption(schemeCode);
  await ensureTimeSemanticSelected(page, '左侧时间口径');
  await ensureTimeSemanticSelected(page, '右侧时间口径');
