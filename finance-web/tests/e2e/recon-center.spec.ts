...
function extractValidationSummary(value: Record<string, unknown>, section: 'proc' | 'recon'): Record<string, unknown> {
  const summary = value.validation_summary || value.validationSummary;
  if (!summary || typeof summary !== 'object') return {};
  const row = (summary as Record<string, unknown>)[section];
...
async function createReconScheme(
  ...
  await expect(page.getByText('按三步完成方案设计与试跑确认')).toBeVisible();
  ...
  await expect(page.getByText('匹配字段')).toBeVisible();
  await expect(page.getByText('对比字段')).toBeVisible();
  await expect(page.getByRole('button', { name: '查看 JSON' })).toBeEnabled();
  ...
  await expect(page.getByText('按三步完成方案设计与试跑确认')).toHaveCount(0, { timeout: 30_000 });
  ...
  const reconValidationSummary = extractValidationSummary(savedSchemeMeta, 'recon');
  expect(String(savedSchemeMeta.recon_trial_status || '')).toBe('structure_checked');
  expect(String(savedSchemeMeta.recon_trial_summary || '')).toBe('已完成对账规则结构校验，待首次真实运行验证');
  expect(String(reconValidationSummary.status || '')).toBe('structure_checked');
...
test('对账方案可完成真实生成并保存运行计划后查看运行记录', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '保存方案' }).click();
  await expect(page.getByText('按三步完成方案设计与试跑确认')).toHaveCount(0, { timeout: 30_000 });
  console.log('Step 3 save scheme done');
  ...
  expect(String(savedSchemeMeta.recon_trial_summary || '')).toBe('已完成对账规则结构校验，待首次真实运行验证');
  expect(String(reconValidationSummary.status || '')).toBe('structure_checked');
  expect(String(reconValidationSummary.summary || '')).toBe('已完成对账规则结构校验，待首次真实运行验证');
