...
def test_plan_create_rejects_enabled_daily_plan_when_rollup_cannot_be_inferred(monkeypatch) -> None:
    monkeypatch.setattr(
        execution_runs,
        "_require_user",
        lambda _token: {"company_id": "co-1", "user_id": "user-1"},
    ...
    assert result == {"success": False, "error": "无法从对账规则推导日报 rollup 字段映射"}
