...
class DouyinShopConnector(BasePlatformConnector):
    ...
    def _authorize_url(self) -> str:
        return str(
            ...
            or "https://op.jinritemai.com/authorization/store"
        )
        ...
        *,
        ...
        if self.is_mock:
            ...
            return PlatformTokenBundle(
                access_token=f"mock_dy_access_{suffix}",
                refresh_token=f"mock_dy_refresh_{suffix}",
                expires_in=7200,
                refresh_expires_in=30 * 24 * 3600,
                scope_text="order,shop,refund",
                raw_payload={"mode": "mock", "code": code},
            )
        raise NotImplementedError("抖店真实换 token 流程待接入真实平台应用参数")
    ...
    def refresh_token(self, *, refresh_token: str) -> PlatformTokenBundle:
        ...
        raise NotImplementedError("抖店真实 refresh token 流程待接入")
    ...
    def fetch_shop_profile(
        ...
        *,
        ...
        if self.is_mock:
            ...
            shop_id = str(payload.get("mock_shop_id") or f"dy_shop_{session_id}")
            shop_name = str(payload.get("mock_shop_name") or f"抖店测试店铺{session_id}")
            seller_id = str(payload.get("mock_seller_id") or f"dy_seller_{session_id}")
            return PlatformShopProfile(
                ...
                shop_type=str(payload.get("mock_shop_type") or "virtual_goods"),
                metadata={"mode": "mock", "platform": "douyin_shop"},
            )
        raise NotImplementedError("抖店真实店铺信息查询待接入")
